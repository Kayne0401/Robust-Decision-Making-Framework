...
"duration": 800,  # 单次episode的最多step  测试时为800，训练时改成200
...
if __name__ == '__main__':
    ...
    save_path = 'out/HighwayEnv/SAC_SB/Train_{}_{}/SACAgent'.format(time_now, os.getpid())
    excel = xlsxwriter.Workbook('out/HighwayEnv/SAC_SB/Test_{}_{}/record.xlsx'.format(time_now, os.getpid()))
    ...
    if TRAIN:
        seed = 2
        log_path = 'out/HighwayEnv/SAC_SB/Train_{}_{}'.format(time_now, os.getpid())
    else:
        seed = 3
        log_path = 'out/HighwayEnv/SAC_SB/Test_{}_{}'.format(time_now, os.getpid())
    ...
    if TRAIN:
        ...
        load_path = save_path
        # del model
        sheet.write(0, 13, load_path)
        sys.exit()
    else:
        load_path = 'out/HighwayEnv/SAC_SB/Train_20220706-095814_29444/SACAgent'
        sheet.write(0, 13, load_path) 
    ...
    for episode in range(120):
        ...
        mean_reward = ep_reward / 800 # ep_step
